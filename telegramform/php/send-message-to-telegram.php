...
// Токен
  const TOKEN = '7750061649:AAE2KnKZvVBk_xvs-fwK1qAx78A0QR0DAs0';
 
  // ID чата
  const CHATID = '-4517421375';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST['name']) && !empty($_POST['phone']) && !empty($_POST['surname'])) {
    ...
    if (isset($_POST['name']) && !empty($_POST['name'])) {
        $txt .= "Имя пославшего: " . strip_tags(trim(urlencode($_POST['name']))) . strip_tags(trim(urlencode($_POST['surname']))) . "%0A";
    }
